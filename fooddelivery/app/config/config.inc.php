<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "root");
if (!defined("PJ_PASS")) define("PJ_PASS", "Hanad20202030/*-+");
if (!defined("PJ_DB")) define("PJ_DB", "foodd");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "script_");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/fooddelivery/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "/var/www/html/fooddelivery/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/fooddelivery/");
if (!defined("PJ_SALT")) define("PJ_SALT", "CIITPDI2");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>