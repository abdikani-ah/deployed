...
if (isset($tpl['locale_arr']) && is_array($tpl['locale_arr']) && !empty($tpl['locale_arr']) && count($tpl['locale_arr']) > 1)
{
	...
	<div class="btn-group pull-right pjRbLaunguage">
		<button type="button" class="btn dropdown-toggle pjRbBtnLaunguageTrigger" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo $selected_title;?></button>
		...
		<ul class="dropdown-menu">
			...
			{ 
				...
				<li class="<?php echo $locale_id == $locale['id'] ? 'current' : NULL; ?>">
					<a href="#" class="rbSelectorLocale" data-id="<?php echo $locale['id']; ?>"><?php echo pjSanitize::html($locale['name']); ?><img src="<?php echo $selected_src; ?>" alt=""></a>
				</li>
				<?php
			} 
			?>
