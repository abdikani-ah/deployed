            ...
            if (pjAuth::factory('pjAdminOptions', $this->_post->toString('next_action'))->hasAccess())
            {
                ...
                switch ($this->_post->toString('next_action'))
                {
                    ...
                    case 'pjActionTerm':
                        $err = 'AO05';
                        break;
                    case 'pjActionReminder':
                        $is_reminder_enabled = $pjOptionModel
                            ->reset()
                            ->where('foreign_id', $this->getForeignId())
                            ->where('`key`', 'o_reminder_enable')
                            ->where('value', 'Yes|No::Yes')
                            ->findCount()
                            ->getData();
						...
						$is_reminder_sms_enabled = $pjOptionModel
                            ->reset()
                            ->where('foreign_id', $this->getForeignId())
                            ->where('`key`', 'o_reminder_enable_sms')
                            ->where('value', 'Yes|No::Yes')
                            ->findCount()
                            ->getData();
                        ...
                        pjBaseCronJobModel::factory()->setIsActive($active, 'pjCron', 'pjActionReminder');
